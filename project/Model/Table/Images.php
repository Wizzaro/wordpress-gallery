...
class Images extends AbstractModel {
    ...
    public function save_image_data( $img_id, $data ) {
        global $wpdb;
        $data = array(
            'alt_text' => isset( $data['alt_text'] ) && mb_strlen( $data['alt_text'] ) > 0 ? $data['alt_text'] : null,
            'description' => isset( $data['description'] ) && mb_strlen( $data['description'] ) > 0 ? $data['description'] : null
        );

        $wpdb->update( $this->_get_table_prefix(), $data, array( 'ID' => $img_id ), array( '%s', '%s' ), array( '%d' ) );
    }
